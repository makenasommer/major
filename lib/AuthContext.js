...
export function AuthProvider({ children }) {
  ...
  async function updateProfile({ username, photoFile }) {
    if (!auth.currentUser) return;

    let photoURL = user?.photoURL || null;
    if (photoFile) {
      const storageRef = ref(storage, `avatars/${auth.currentUser.uid}/${Date.now()}-${photoFile.name}`);
      await uploadBytes(storageRef, photoFile);
      photoURL = await getDownloadURL(storageRef);
    }

    const userRef = doc(db, "users", auth.currentUser.uid);
    const updated = { ...user, username: username ?? user?.username ?? "", photoURL };
    await setDoc(userRef, updated);
    setUser(updated);
  }
